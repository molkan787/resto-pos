...
module.exports = class Changes {
    ...
    async init(){
        ...
        if(this.metadata2.isNewSetup){
            await this.setCurrentStamps(this.db2, this.metadata2);
        }
    }
    ...
    async getTableChanges(db, metadata, table){
        ...
        const newRows = await db.select().from(table)
                        .where(idColumn, '>', lastId).orderBy(idColumn, 'asc')
                        .whereRaw(MetaData.REF_STATEMENT)
                        .limit(this.maxRowsCount);
        const updatedRows = await db.select().from(table)
                        .where(function () {
                            this.where(self._getTableUpdatedColumn(table), '>', metadata.getLastUpdateTime(table))
                            .orWhere(MetaData.DELETED_TIME_COLUMN, '>', metadata.getLastUpdateTime(table))
                        })
